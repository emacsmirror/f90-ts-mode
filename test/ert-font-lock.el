...
;; Modified for f90-ts-mode by Martin Stein, 2026:
;; * Processing changed, so that assertions are removed before font locking
;;   is done, so that the comment lines containing the assertions do not
;;   interfer with tree-sitter file parsing, which was observed during
;;   error recovery (rendering error node overlay tests invalid).
;; * Added to repo to allow testing with emacs 29.3, which does not contain
;;   this file.
...
Argument TEST-NAME - name of the currently running ert test."
  (ert-font-lock-test-string str mode)
  test-name)
      ...
      (catch 'nextline
        ...
        (when (re-search-forward ert-font-lock--assertion-line-re
          ...
          (let* (;; either comment start char column (for arrows) or
                 ...
                 ;; the own text of the assertion, captured now so it
                 ;; remains available for failure reports after the
                 ;; assertion line is deleted (see `ert-font-lock--delete-lines')
                 (assert-str (ert-font-lock--get-line curline)))

            ;; Collect the first assertion on the line
            (push (list :line-checked linetocheck
                        :line-assert curline
                        :line-assert-str assert-str
...
(defun ert-font-lock--delete-lines (lines)
  "Delete the text content of LINES.
LINES is a list if line numbers (first line in buffer has number 1).

Each line's terminating newline is left in place, so the line numbers
of the rest of the buffer are unaffected by the erasure.

This is used to strip assertion comments out of the buffer before it
is fontified, so the assertions themselves cannot influence how the
surrounding code is fontified.  This matters in particular for tests
targeting error-node recovery in tree-sitter grammars. The trailing
assertions (even if just comments) sometimes alter the recover path
of the tree-sitter parser and can swallow the ERROR node or change
the CST surrounding the expected ERROR node."
  (dolist (line lines)
    (save-excursion
      (goto-char (point-min))
      (forward-line (1- line))
...
The function is meant to be run from within an ERT test."
  (dolist (test tests)
    (let* ((line-checked (plist-get test :line-checked))
           ...
           ;; The assertion line was deleted from the buffer before
           ;; fontification, so its text is taken from the test
           ;; itself rather than re-read from the buffer.
           (line-assert-str (plist-get test :line-assert-str)))
...
Assertion comments are parsed out and then deleted from the buffer
before fontification, so they cannot themselves affect the
fontification of the code under test (see `ert-font-lock--delete-lines').
...
The function is meant to be run from within an ERT test."
  ...
  (with-temp-buffer
    ...
    (let* ((tests (ert-font-lock--parse-comments))
           (assertion-lines (delete-dups
                              (mapcar (lambda (test) (plist-get test :line-assert))
                                      tests))))
      (ert-font-lock--delete-lines assertion-lines)
      (font-lock-ensure)
